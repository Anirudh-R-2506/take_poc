        ...
        if (bluetoothEnabled) {
            ...
            if (pairedDevices != nil) {
                for (IOBluetoothDevice* device in pairedDevices) {
                    ...
                    // Check if device is connected - CRITICAL: Only flag actively connected devices
                    bool isConnected = [device isConnected];

                    // SKIP paired but disconnected devices - they're not a current threat
                    if (!isConnected) {
                        continue;
                    }

                    // Only process connected devices with valid names and addresses
                    if (!deviceName.empty() && deviceName != "Unknown Device" &&
                        !deviceAddress.empty() && deviceAddress != "Unknown Address") {
