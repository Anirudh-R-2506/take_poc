...
#ifdef _WIN32
    if (!examActive_.load()) return false;
...
bool NotificationBlocker::SetFocusAssistState(int state) {
    try {
        // Method 1: Try using the notification state API
        // This is read-only, so we need to use registry approach

        // Method 2: Registry approach for Focus Assist
...
std::string NotificationBlocker::StateToString(NotificationBlockState state) {
    switch (state) {
        case NotificationBlockState::DISABLED: return "disabled";
        case NotificationBlockState::ENABLED: return "enabled";
        case NotificationBlockState::ERROR: return "error";
